...
public sealed class ConstructorForm : Form
{
    ...
    public ConstructorForm(SessionUser user, long? testId)
    {
        ...
        addQ.Click += (_, _) => AddQuestion(); del.Click += (_, _) => DeleteQuestion(); up.Click += (_, _) => MoveQuestion(-1); down.Click += (_, _) => MoveQuestion(1); save.Click += (_, _) => SaveDraft();
    ...
    private void MoveQuestion(int dir) { }
