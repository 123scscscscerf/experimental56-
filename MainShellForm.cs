...
public sealed class MainShellForm : Form
{
    ...
    private readonly Panel _content = new() { Dock = DockStyle.Fill, Padding = new Padding(12), BackColor = AppTheme.Bg };
    public bool ShouldRelogin { get; private set; }

    public MainShellForm(SessionUser user)
    {
        _user = user;
        Text = "Testing Platform";
    ...
    {
        var p = new Panel { Dock = DockStyle.Top, Height = 56, BackColor = AppTheme.Panel, Padding = new Padding(12) };
        var lbl = new Label { Text = $"{_user.DisplayName} ({_user.Role})", AutoSize = true, Location = new Point(12, 17) };
        var change = AppTheme.FlatButton("Сменить пароль"); change.Width = 150; change.Location = new Point(Width - 350, 10); change.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        change.Click += (_, _) => { using var f = new ChangePasswordForm(_user); f.ShowDialog(); };
        var exit = AppTheme.FlatButton("Выйти"); exit.Width = 90; exit.Location = new Point(Width - 180, 10); exit.Anchor = AnchorStyles.Top | AnchorStyles.Right;
        exit.Click += (_, _) => { ShouldRelogin = true; Close(); };
        p.Controls.Add(lbl); p.Controls.Add(change); p.Controls.Add(exit);
        return p;
    }
    ...
    private Control BuildMenu()
    {
        ...
        {
            Add("Users", () => Open(new AdminUsersPanel(_user)));
            Add("Groups", () => Open(new AdminGroupsPanel(_user)));
            Add("Audit", () => Open(new AdminAuditPanel()));
        }
        ...
        {
            Add("Available Tests", () => Open(new StudentAvailablePanel(_user)));
            Add("My Attempts", () => Open(new StudentAttemptsPanel(_user)));
        }
...
public sealed class AdminGroupsPanel : UserControl
{
    ...
    public AdminGroupsPanel(SessionUser user)
    {
        ...
        addG.Click += (_, _) => { var n = Prompt("Group name"); if (string.IsNullOrWhiteSpace(n)) return; DataAccess.Execute("INSERT INTO Groups(Name) VALUES(@n)", ("@n", n)); LoadGroups(); };
    ...
    private long GroupId() => _groups.SelectedItem is null ? 0 : ((dynamic)_groups.SelectedItem).Id;
...
public sealed class TeacherTestsPanel : UserControl
{
    ...
    public TeacherTestsPanel(SessionUser user)
    {
        ...
        create.Click += (_, _) => { using var f = new ConstructorForm(_user, null); f.ShowDialog(); LoadData(); };
        open.Click += (_, _) => { var id = Id(); if (id == 0) return; using var f = new ConstructorForm(_user, id); f.ShowDialog(); LoadData(); };
    ...
    private void Add()
    {
        ...
        b.Click += (_, _) =>
        {
            DataAccess.Execute("INSERT INTO Assignments(TestId,TargetType,TargetId,AvailableFrom,Deadline,AttemptLimit,TimeLimitMinutes,ShuffleQuestions,ShuffleOptions,ShowScoreAfter,ShowCorrectAfter,IsActive) VALUES(@t,@tt,@ti,@af,@dl,@al,@tl,@sq,@so,@ss,@sc,1)",
                ("@t", ((DataRowView)test.SelectedItem)["Id"]), ("@tt", type.Text), ("@ti", Convert.ToInt64(target.Text)), ("@af", DateTime.UtcNow.ToString("O")), ("@dl", DateTime.UtcNow.AddMonths(1).ToString("O")), ("@al", lim.Value), ("@tl", tl.Value), ("@sq", s1.Checked ? 1 : 0), ("@so", s2.Checked ? 1 : 0), ("@ss", sc.Checked ? 1 : 0), ("@sc", cc.Checked ? 1 : 0));
            f.Close(); LoadData();
        };
...
WHERE a.IsActive=1 AND t.Status='Published' AND ((a.TargetType='User' AND a.TargetId=@u) OR (a.TargetType='Group' AND gm.UserId=@u))", ("@u", _user.Id));
    ...
    private void StartAttempt()
    {
        ...
        try
        {
            ...
            long used;
            using (var c2 = c.CreateCommand()) { c2.Transaction = tx; c2.CommandText = "SELECT COUNT(*) FROM Attempts WHERE AssignmentId=@a AND UserId=@u"; c2.Parameters.AddWithValue("@a", assignmentId); c2.Parameters.AddWithValue("@u", _user.Id); used = (long)c2.ExecuteScalar()!; }
            if (used >= limit) throw new Exception("Лимит попыток");
            using (var c3 = c.CreateCommand()) { c3.Transaction = tx; c3.CommandText = "SELECT COUNT(*) FROM Attempts WHERE AssignmentId=@a AND UserId=@u AND Status='Active'"; c3.Parameters.AddWithValue("@a", assignmentId); c3.Parameters.AddWithValue("@u", _user.Id); if ((long)c3.ExecuteScalar()! > 0) throw new Exception("Есть активная попытка"); }
            ...
            ins.CommandText = "INSERT INTO Attempts(AssignmentId,UserId,StartedAt,EndsAt,Status,SnapshotJson) VALUES(@a,@u,@st,@en,'Active',@s)";
            ins.Parameters.AddWithValue("@a", assignmentId); ins.Parameters.AddWithValue("@u", _user.Id); ins.Parameters.AddWithValue("@st", DateTime.UtcNow.ToString("O")); ins.Parameters.AddWithValue("@en", DateTime.UtcNow.AddMinutes(tl).ToString("O")); ins.Parameters.AddWithValue("@s", snap);
            ins.ExecuteNonQuery();
            var attemptId = c.LastInsertRowId;
            tx.Commit();
